...
import datetime, os, tarfile, time, subprocess, shutil, smtplib, logging, signal, sys
...
Your experiment {expname} has completed processing and available for download.
...
NOTE: 
All times are according to UTC-05:00 (Eastern Time Zone).
...
def process_graple_results(uid, retention):
    ...
    if 'email' in dbdoc and len(dbdoc['email']) > 0:
        ...
        emailmsg = MIMEText(email_template.format(expname = expName,
            expid = dbdoc['key'],
            subtime = dbdoc['submitted'].ctime(),
            reslink = serv_dl_addr + dbdoc['key'],
            comptime = dbdoc['completed'].ctime(),
            exptime = dbdoc['expiry'].ctime() if 'expiry' in dbdoc else "On first download"))
        emailmsg['From'] = "Graple Notifier"
        ...
        smtpserv.sendmail(emailmsg['From'], [emailmsg['To']], emailmsg.as_string())
        smtpserv.quit()
